...
const recipes = [
  {
    ...
    carbs: 15,
    fat: 12,
    instructions: 'Sauté garlic, onion, and ginger. Add chicken and cook until brown. Add water and calabaza. Simmer until tender. Add malunggay leaves.',
    prepTime: '45 mins',
    difficulty: 'Easy'
  },
  {
    ...
    carbs: 35,
    fat: 18,
    instructions: 'Cook tuna with vegetables. Fill pie crust. Top with cheese. Bake at 375°F for 30 mins.',
    prepTime: '60 mins',
    difficulty: 'Medium'
  },
  {
    ...
    carbs: 25,
    fat: 4,
    instructions: 'Boil tamarind soup base. Add tomatoes and onion. Add vegetables. Simmer until tender.',
    prepTime: '40 mins',
    difficulty: 'Easy'
  },
  {
    ...
    carbs: 20,
    fat: 25,
    instructions: 'Brown beef cubes. Add tomato sauce and simmer. Add vegetables. Top with cheese before serving.',
    prepTime: '90 mins',
    difficulty: 'Medium'
  },
  {
    ...
    carbs: 45,
    fat: 8,
    instructions: 'Cook noodles. Stir-fry meat and vegetables. Mix with noodles. Season with soy sauce.',
    prepTime: '30 mins',
    difficulty: 'Easy'
  }
];
...
export default function RecipeScreen({ navigation }) {
  ...
  const loadUserMeals = async () => {
    try {
      ...
      if (!user) { setRecommendedRecipes(recipes.slice(0, 3)); return; }
      const q = query(collection(db, 'users', user.uid, 'meals'), orderBy('date', 'desc'), limit(10));
  ...
  const generateRecommendations = (meals) => {
    ...
    let recommendations = [];
    if (recentFoods.some(f => f.includes('chicken'))) recommendations.push(recipes.find(r => r.name === 'Chicken Tinola'));
    if (recentFoods.some(f => f.includes('fish') || f.includes('tuna'))) recommendations.push(recipes.find(r => r.name === 'Tuna Pie'));
    if (recentFoods.some(f => f.includes('vegetable'))) recommendations.push(recipes.find(r => r.name === 'Vegetable Sinigang'));
    if (recommendations.length === 0) recommendations = recipes.slice(0, 3);
    ...
    >
      <View style={styles.recipeHeader}>
        ...
        <View style={styles.recipeMeta}>
          <Text style={[styles.recipeTime, { color: theme.subtext }]}>{recipe.prepTime}</Text>
      ...
      <View style={styles.ingredientsPreview}>
        <Text style={[styles.ingredientsText, { color: theme.subtext }]}>
...
const styles = StyleSheet.create({
  ...
  recipeCard: { borderRadius: 16, padding: 16, marginBottom: 12, elevation: 3, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 4 },
