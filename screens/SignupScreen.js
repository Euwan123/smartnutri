...
export default function SignupScreen({ navigation }) {
  ...
  const nextStep = () => { if (!name || !email) return Alert.alert('Error', 'Please fill in all fields'); animateStep(); setStep(2); };
  ...
  const signup = async () => {
    ...
    try {
      ...
      await setDoc(doc(db, 'users', result.user.uid), { name, email: email.trim(), createdAt: new Date().toISOString(), calorieGoal: 2000, location: 'Davao City', isPublic: true });
    } catch (e) {
      Alert.alert('Signup Failed', e.code === 'auth/email-already-in-use' ? 'Email already registered' : e.message);
    }
    setLoading(false);
  };
      ...
      <ScrollView style={{ flex: 1 }} contentContainerStyle={{ flexGrow: 1 }} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false} bounces={false}>
        ...
        <Animated.View style={[styles.form, { transform: [{ translateY: slideAnim }] }]}>
          {step === 1 ? (
            <>
              ...
              <View style={[styles.inputWrapper, focusedField === 'name' && { borderColor: theme.primary, backgroundColor: theme.light }]}>
                ...
                <TextInput style={styles.input} placeholder="Juan Dela Cruz" placeholderTextColor="#bbb" value={name} onChangeText={setName} onFocus={() => setFocusedField('name')} onBlur={() => setFocusedField(null)} />
              </View>
              ...
              <View style={[styles.inputWrapper, focusedField === 'email' && { borderColor: theme.primary, backgroundColor: theme.light }]}>
                ...
                <TextInput style={styles.input} placeholder="you@email.com" placeholderTextColor="#bbb" keyboardType="email-address" autoCapitalize="none" value={email} onChangeText={setEmail} onFocus={() => setFocusedField('email')} onBlur={() => setFocusedField(null)} />
              </View>
            ...
            <>
              ...
              <View style={[styles.inputWrapper, focusedField === 'password' && { borderColor: theme.primary, backgroundColor: theme.light }]}>
                ...
                <TextInput style={styles.input} placeholder="Min 6 characters" placeholderTextColor="#bbb" secureTextEntry={!showPass} value={password} onChangeText={setPassword} onFocus={() => setFocusedField('password')} onBlur={() => setFocusedField(null)} />
              ...
              <View style={[styles.inputWrapper, focusedField === 'confirm' && { borderColor: theme.primary, backgroundColor: theme.light }]}>
                ...
                <TextInput style={styles.input} placeholder="Repeat password" placeholderTextColor="#bbb" secureTextEntry={!showPass} value={confirm} onChangeText={setConfirm} onFocus={() => setFocusedField('confirm')} onBlur={() => setFocusedField(null)} />
              </View>
              <TouchableOpacity style={[styles.btn, { backgroundColor: theme.primary }, loading && styles.btnDisabled]} onPress={signup} disabled={loading}>
          ...
          <TouchableOpacity onPress={() => navigation.navigate('Login')} style={styles.switchBtn}>
            <Text style={styles.switchText}>Already have an account? <Text style={[styles.switchLink, { color: theme.primary }]}>Sign In</Text></Text>
          </TouchableOpacity>
